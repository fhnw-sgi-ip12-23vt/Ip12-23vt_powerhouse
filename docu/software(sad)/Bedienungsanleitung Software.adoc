...
*ioAll.csv:* Dieses CSV befindet sich im Paket (com.pi4j.mvc.powerhouse.components). Hier können die LEDs und Druckknöpfe einander zugeorndet werden. "E + Zahl" sind für die LEDs und "D + Zahl" sind für die Buttons.
...
|pui
|questions.json
|Im gui befinden sich mehrere Schlüsselklassen, die für die Darstellung und das Management der verschiedenen Spielszenen und Elemente verantwortlich sind. Jede dieser Klassen spielt eine spezifische Rolle im Kontext der Benutzeroberfläche des Spiels.
|Die *SimulationObjectPUI* Klasse, die sich im pui-Paket befindet, verwaltet die physische Benutzerinteraktion durch LEDs und Buttons, die den SimulationObjects zugeordnet sind.
|Diese JSON-Datei enthält die Definitionen der Fragen und ist zentral für die Anpassung der Spielinhalte. Durch Änderungen an dieser Datei können die Fragen und Antworten, die im Spiel verwendet werden, modifiziert oder erweitert werden.
|===

==== questions.jason
In diesem Jason-File können Fragen angepasst und neue hinzugefügt werden. Jede Frage benötigt Folgendes:
...
* *ActiveAtStart:* Hier werden die Gegenstände aufgelistet, die zu Beginn des Levels an sein sollten.
